...
(function() {
  const ctlProto = TC.control.LayerCatalogSilmeFolders.prototype;
  ...
  ctlProto.loadTemplates = function () {
    ...
    _this.template[_this.CLASS + "-node"] = "assets/js/patch/templates/LayerCatalogNodeSilmeFolders.hbs";
    _this.template[_this.CLASS + "-branch"] = "assets/js/patch/templates/LayerCatalogBranchSilmeFolders.hbs";
  ...
  ctlProto.register = async function(map) {
    const _this = this;
    const result = await TC.control.LayerCatalog.prototype.register.call(_this, map);
    ...
    map
      .on(TC.Consts.event.LAYERADD + ' ' + TC.Consts.event.UPDATEPARAMS, function(e) {
        ...
        if (!layer.isBase && layer.type === TC.Consts.layerType.WMS) {
          ...
          _this.loaded().then(function() { // Esperamos a que cargue primero las capas de la configuración

            if (_this.getLayerRootNodes(layer)) {
              updateControl.call(_this, layer);
            } else {
              ...
              if (layerAlreadyAdded) {
                if (!_this.layersToSetChecked) {
                  _this.layersToSetChecked = [];
                }

                _this.layersToSetChecked.push(layer);
              } else {
                _this.addLayer(new TC.layer.Raster({
      ...
      .on(TC.Consts.event.EXTERNALSERVICEADDED, function(e) {
        if (e && e.layer) {
          _this.addLayer(e.layer);
          _this.div.classList.remove(TC.Consts.classes.COLLAPSED);
        }
      })

      .on(TC.Consts.event.LAYERREMOVE, function(e) {
        const layer = e.layer;
        _this.getLayerNodes(layer).forEach(function(node) {
          try {
  ...
  ctlProto.hideLayerInfo = function() {
    const _this = this;
    _this.div.querySelectorAll('.' + _this.CLASS + '-btn-info, .' + _this.CLASS + '-search-btn-info').forEach(function(btn) {
      btn.classList.remove(TC.Consts.classes.CHECKED);
    });
    const infoPanel = _this.div.querySelector('.' + _this.CLASS + '-info');
  ...
  ctlProto.getLayerRootNodes = function(layer) {
    const self = this;
    var result = [];
    if (!layer.isBase) {
  ...
  const uncheckParentFoldersIfChildless = function(node) {
    ...
    // Cuando llegamos al 'tc-ctl-lcat-tree' paramos, ya que es la raíz del árbol
    while (!parent.classList.contains('tc-ctl-lcat-tree')) {
      if (parent.classList.contains('sitmun-folder')) {
        ...
        const nodesChecked = parent.querySelectorAll('li.tc-ctl-lcat-leaf.tc-checked');
