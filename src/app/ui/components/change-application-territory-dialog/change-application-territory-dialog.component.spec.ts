...
describe('ChangeApplicationTerritoryDialogComponent', () => {
  ...
  beforeEach(() => {
    const mockCommonService = jasmine.createSpyObj('CommonService', ['fetchDashboardItems', 'fetchTerritoriesByApplication']);
    mockCommonService.fetchDashboardItems.and.returnValue(of({ content: [] }));
    mockCommonService.fetchTerritoriesByApplication.and.returnValue(of({ content: [] }));

    const mockAppConfigService = jasmine.createSpyObj('AppConfigService', ['getDashboardConfig']);
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        { provide: Router, useValue: jasmine.createSpyObj('Router', ['navigate', 'navigateByUrl'], { url: '/user/dashboard', routerState: { snapshot: { root: { firstChild: null, params: {} } } } }) },
