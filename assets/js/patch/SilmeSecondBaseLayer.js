...
async function addSecondBaseLayer() {
    ...
    for (var i = 0; i < silmeMap.parent.baseLayers.length; i++) {
        var found = silmeMap.map.getLayers().array_.some(e => e._wrap.parent === silmeMap.parent.baseLayers[i]);
    ...
    if (secondBaseLayer) {
        ...
        workingMap.insertLayer(workingMap.getLayer("second-1"), getLastBaseMapIndex());
    }
...
function selectB1() {
    ...
    //capa2 = silmeMap.map.getLayers().array_[1];
    capa2 = TC.Map.get(document.querySelector('.tc-map')).getLayer("second-1").wrap.layer;
    capa2Layer = getLayerName(capa2);
...
function styleInputBasemap(branch, capaLayer, color, labelId) {
    ...
    for (var i = 0; i < branch.children.length; i++) {
        if (branch.children[i].attributes[1].value == capaLayer) {
            var label = branch.children[i].children[0];
            if (labelId) lblBasemap.innerHTML = branch.children[i].children[1].children[0].innerText;
